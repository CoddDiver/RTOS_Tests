...
void Profile_SRamp() { // For S ramped movement using Servo for S ramping
	...
	if ((Drive.Fade_Target >= Drive.Min_Limit)
		...
		D = Drive.Fade_Target - Drive.System_position; // Calculate distance to move
	} else {
		D = 0;
...
void M4_Joystick() {
	...
	if (joy_demand <= -(joy_deadband * Drive.Joy_SF)) {
		Drive.sysjogdir = -1;
